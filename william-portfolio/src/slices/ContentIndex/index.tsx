...
const ContentIndex = async ({ slice }: ContentIndexProps): Promise<JSX.Element> => {
  ...
  const hobbyposts = await client.getAllByType("hobby_posts")
  const projects = await client.getAllByType("projects")
  
  const contentType = slice.primary.content_type || "Hobbies"

  const items = contentType === "Hobbies" ? hobbyposts : projects
